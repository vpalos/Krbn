...
- ✅ **Phase-1 polish complete:** consolidation, cylinder/cone surface hatching,
  ...
  **torus** (`src/primitives/torus.ts`: numerical silhouette as two contour loops
  ...
  and **curved hatch direction fields** (`FeatureSource.hatchField` + the scene's
  `clipHatchField`): a primitive emits its exact iso-parameter curves — cylinder
  rings + rulings, cone rings + generators, torus poloidal + toroidal circles — as
  world-space samples with normals, and the scene keeps the runs that are
  front-facing, unoccluded, and dark enough for the tonal layer, so the field
...
the author's quantized importance. Two field regimes, both routed through the same
visibility machinery so hatching stops at occlusion boundaries and its gaps reveal
what is behind (alpha-free transparency):

- ✅ **Straight fields** (flat faces, and the quadrics that don't carry an explicit
  parametrization) — parallel lines clipped to the region's **projected, visible**
  outline (`pipeline/hatch.ts`); the scene back-projects each sample to shade it by
  `N·L` in tonal layers.
- ✅ **Curved iso-parameter fields** (`FeatureSource.hatchField`) — a primitive that
  knows its parametrization emits its own iso-curves (cylinder rings + rulings, cone
  rings + generators, torus poloidal + toroidal circles) as world-space samples with
  normals; the scene's `clipHatchField` keeps the runs that are front-facing,
